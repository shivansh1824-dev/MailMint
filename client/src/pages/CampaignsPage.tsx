...
export const CampaignsPage: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-fadeIn pb-12">
      ...
      <div className="bg-[#161B22] border border-[#2D3A4A] rounded-[8px] p-5 space-y-4">
        ...
        {followups.filter((f) => f.status === 'pending-approval').length === 0 ? (
          <p className="text-[12px] text-[#6B7280] py-2">
          ...
          <div className="space-y-2.5">
            {followups
              ...
              .map((f) => (
                <div
                  key={f.id}
                  className="p-3 bg-[#111827] border border-[#2D3A4A] rounded-[6px] flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-[12px]"
                >
                  <div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-3 py-1 bg-[#161B22] hover:bg-[#1F2937] text-[#9CA3AF] hover:text-white rounded border border-[#2D3A4A]"
                    >
                      Dismiss
                    </button>
                    <button
                      onClick={() => handleApproveFollowup(f.id)}
                      className="px-4 py-1 bg-[#00C896] text-[#0D1117] font-bold rounded hover:bg-[#00b084] shadow-sm"
                    >
                      Approve & Send
                    </button>
                  </div>
      ...
      <div className="space-y-4">
        <h2 className="text-[18px] font-bold text-white font-sans">Active Campaigns</h2>

        {campaigns.length === 0 ? (
          <div className="p-12 text-center bg-[#161B22] border border-[#2D3A4A] rounded-[8px] space-y-3">
            <Send className="w-10 h-10 text-[#6B7280] mx-auto" />
            <h3 className="text-[16px] font-semibold text-white">No campaigns yet</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {campaigns.map((c) => (
              <div
                ...
                className="p-5 bg-[#161B22] border border-[#2D3A4A] hover:border-[#00C896]/40 rounded-[8px] cursor-pointer transition-all space-y-4"
              >
                ...
                <div className="grid grid-cols-3 gap-2 p-2.5 bg-[#111827] rounded text-[11px]">
                  <div>
                    <span className="text-[#6B7280] block">Contacts</span>
                    <span className="text-white font-mono font-bold">
                      {c.contact_ids?.length || 0}
                    </span>
                  </div>
                  <div>
                    <span className="text-[#6B7280] block">Sent</span>
                    <span className="text-white font-mono font-bold">
                      {c.stats?.sent || 0}
                    </span>
                  </div>
                  <div>
                    <span className="text-[#6B7280] block">Replies</span>
                    <span className="text-[#00C896] font-mono font-bold">
                      {c.stats?.replies || 0}
                    </span>
                  </div>
                </div>
      ...
      {selectedCampaign && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex justify-end animate-fadeIn">
          <div className="w-full max-w-xl bg-[#161B22] border-l border-[#2D3A4A] h-full flex flex-col p-6 overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between pb-4 border-b border-[#2D3A4A] mb-4">
              <div>
                <h2 className="text-[20px] font-bold text-white">{selectedCampaign.name}</h2>
              ...
              <div className="space-y-6">
                ...
                {campaignStats.winner && (
                  <div className="p-4 rounded-[8px] bg-emerald-500/10 border border-emerald-500/30 flex items-center gap-3">
                    <Trophy className="w-6 h-6 text-[#00C896] flex-shrink-0" />
                    <div>
                ...
                <div className="space-y-3">
                  <h3 className="text-[14px] font-bold text-white flex items-center gap-2">
                    <Split className="w-4 h-4 text-[#00C896]" />
                    <span>A/B Variant Comparison</span>
                  </h3>

                  <div className="grid grid-cols-2 gap-4">
                    {/* Variant A */}
                    <div className="p-4 bg-[#111827] border border-[#2D3A4A] rounded-[8px] space-y-2">
                      <div className="flex items-center justify-between text-[12px]">
                        <span className="font-bold text-white">Template A</span>
                        <span className="font-mono text-[#00C896] font-bold text-[14px]">
                    ...
                    <div className="p-4 bg-[#111827] border border-[#2D3A4A] rounded-[8px] space-y-2">
                      <div className="flex items-center justify-between text-[12px]">
                        <span className="font-bold text-white">Template B</span>
      ...
      {createModalOpen && (
        <div className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fadeIn overflow-y-auto">
          <div className="w-full max-w-xl bg-[#161B22] border border-[#2D3A4A] rounded-[8px] shadow-2xl p-6 relative my-8">
            <button
            ...
            <h2 className="text-[18px] font-bold text-white mb-4">Create Outreach Campaign</h2>

            <form onSubmit={handleCreateCampaign} className="space-y-4 text-[13px]">
              <div>
                <label className="block text-[#9CA3AF] mb-1">Campaign Name *</label>
                <input
                  type="text"
                  required
                  value={name}
              ...
              <div>
                <label className="block text-[#9CA3AF] mb-1">Description</label>
                <input
                  type="text"
                  value={description}
              ...
              {/* A/B Test Toggle */}
              <div className="p-3 bg-[#111827] border border-[#2D3A4A] rounded-[6px] space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    <span className="font-semibold text-white block">Enable 50/50 A/B Split Test</span>
                ...
                {abEnabled && (
                  <div className="grid grid-cols-2 gap-3 pt-2 border-t border-[#2D3A4A]">
                    <div>
                      <label className="block text-[11px] text-[#9CA3AF] mb-1">Template A *</label>
                      <select
                        required
                        value={templateAId}
                        onChange={(e) => setTemplateAId(e.target.value)}
                        className="w-full px-2 py-1 bg-[#161B22] border border-[#2D3A4A] rounded text-white outline-none"
                      >
                    ...
                    <div>
                      <label className="block text-[11px] text-[#9CA3AF] mb-1">Template B *</label>
                      <select
                        required
                        value={templateBId}
              ...
              {/* Assign Contacts */}
              <div>
                <label className="block text-[#9CA3AF] mb-1">
                  Assign Contacts ({selectedContactIds.length} selected)
                </label>
                <div className="max-h-36 overflow-y-auto p-2 bg-[#111827] border border-[#2D3A4A] rounded space-y-1">
                  {contacts.map((c) => (
                    <label key={c.id} className="flex items-center gap-2 p-1.5 hover:bg-[#161B22] rounded cursor-pointer">
                      <input
                        type="checkbox"
                        ...
                        className="w-3.5 h-3.5 accent-[#00C896]"
                      />
                      <span className="text-white text-[12px]">{c.name}</span>
                      <span className="text-[#6B7280] text-[11px]">({c.company || 'No company'})</span>
                    </label>
                  ))}
                  {contacts.length === 0 && (
                    <span className="text-[#6B7280] text-[12px]">No contacts available</span>
                  )}
                </div>
              </div>
