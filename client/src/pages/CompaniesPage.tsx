    ...
    <div className="space-y-6 animate-fadeIn pb-12">
      ...
      {isLoading ? (
        ...
        <div className="p-12 text-center bg-[#161B22] border border-[#2D3A4A] rounded-[8px] space-y-3">
          ...
          <h3 className="text-[16px] font-semibold text-white">No companies saved yet</h3>
            ...
            <tbody className="divide-y divide-[#2D3A4A]/50">
              {filtered.map((comp) => (
                <tr
                  ...
                  className="hover:bg-[#1F2937] transition-colors cursor-pointer"
                >
                  <td className="py-3.5 px-4 font-semibold text-white flex items-center gap-2">
                    <Building2 className="w-4 h-4 text-[#00C896] flex-shrink-0" />
                    <span>{comp.name}</span>
                  </td>
                  <td className="py-3.5 px-4 text-[#9CA3AF]">{comp.industry || 'Tech'}</td>
                  <td className="py-3.5 px-4 text-[#9CA3AF]">{comp.location || 'Remote'}</td>
                  <td className="py-3.5 px-4 text-[#9CA3AF] font-mono text-[12px]">
                    {comp.size || '—'}
                  </td>
                  <td className="py-3.5 px-4 text-center font-mono">
                    <span className="px-2 py-0.5 rounded bg-[#111827] text-white text-[12px]">
                  ...
                  <td className="py-3.5 px-4 text-right" onClick={(e) => e.stopPropagation()}>
                    <button
                      onClick={() => openCompanyDetail(comp)}
                      className="px-2.5 py-1 text-[11px] font-semibold bg-[#111827] text-white hover:bg-[#00C896] hover:text-[#0D1117] rounded border border-[#2D3A4A] transition-colors"
                    >
                      Research & Detail
                    </button>
                  </td>
          ...
          <div className="w-full max-w-xl bg-[#161B22] border-l border-[#2D3A4A] h-full flex flex-col p-6 overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between pb-4 border-b border-[#2D3A4A] mb-4">
              <div>
                ...
                <div className="flex items-center gap-3 text-[12px] text-[#9CA3AF] mt-1">
                  {selectedCompany.website && (
                    <a
                      href={selectedCompany.website.startsWith('http') ? selectedCompany.website : `https://${selectedCompany.website}`}
                      target="_blank"
                      rel="noreferrer"
                      className="hover:text-white flex items-center gap-1 text-[#00C896]"
                    >
            ...
            <div className="p-4 bg-[#111827] border border-[#00C896]/30 rounded-[8px] space-y-3 mb-6">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-3 py-1 bg-[#00C896] text-[#0D1117] font-bold text-[12px] rounded hover:bg-[#00b084] disabled:opacity-50 transition-colors"
                >
              ...
              {researchData ? (
                <div className="space-y-3 pt-2 text-[12px] animate-fadeIn">
                  <div className="grid grid-cols-2 gap-2 p-2.5 bg-[#161B22] rounded border border-[#2D3A4A]">
                    <div>
                      <span className="text-[#6B7280] block text-[11px]">Estimated Size</span>
                      <span className="text-white font-medium">{researchData.estimatedSize}</span>
                    </div>
                    <div>
                      <span className="text-[#6B7280] block text-[11px]">Funding Stage</span>
                      <span className="text-white font-medium">{researchData.fundingStage}</span>
                    </div>
                  </div>
                  ...
                  {researchData.recentNews?.length > 0 && (
                    <div className="space-y-1.5 pt-1">
                      <span className="text-[11px] font-mono uppercase text-[#00C896]">Recent News</span>
                      {researchData.recentNews.map((news: any, idx: number) => (
                        <div key={idx} className="p-2 bg-[#161B22] rounded text-[11px] text-[#F0F0F0]">
                          <div className="font-semibold text-white">{news.title}</div>
                          <div className="text-[#6B7280] font-mono text-[10px] mt-0.5">
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-[#2D3A4A]">
                    <span className="text-[10px] font-mono text-[#F59E0B]">
                      {researchData.disclaimer}
                    </span>
                    <button
                      onClick={handleCopyToNotes}
                      className="flex items-center gap-1 text-[11px] text-[#00C896] hover:underline"
                    >
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="space-y-2">
                {companyContacts.map((c) => (
                  <div
                    key={c.id}
                    className="p-3 bg-[#111827] border border-[#2D3A4A] rounded-[6px] flex items-center justify-between text-[12px]"
                  >
                    <div>
                      <div className="font-medium text-white">{c.name}</div>
                      <div className="text-[#6B7280]">{c.job_title}</div>
                    </div>
                    <span className="text-[#9CA3AF] font-mono text-[11px]">{c.email}</span>
                  </div>
                ))}
                {companyContacts.length === 0 && (
                  <div className="text-center py-4 text-[12px] text-[#6B7280]">
            ...
            <div className="space-y-3">
              <h3 className="text-[14px] font-bold text-white flex items-center gap-2">
                <Briefcase className="w-4 h-4 text-[#00C896]" />
              ...
              <div className="space-y-2">
                {companyJobs.map((j) => (
                  <div
                    key={j.id}
                    className="p-3 bg-[#111827] border border-[#2D3A4A] rounded-[6px] text-[12px]"
                  >
                    <div className="font-medium text-white">{j.title}</div>
                    <div className="text-[#6B7280]">{j.location} · {j.employment_type}</div>
                  </div>
                ))}
      ...
      {createModalOpen && (
        <div className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fadeIn">
          <div className="w-full max-w-lg bg-[#161B22] border border-[#2D3A4A] rounded-[8px] shadow-2xl p-6 relative">
            ...
            <h2 className="text-[18px] font-bold text-white mb-4">Add Target Company</h2>

            <form onSubmit={handleCreateCompany} className="space-y-3 text-[13px]">
              <div>
                <label className="block text-[#9CA3AF] mb-1">Company Name *</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[#9CA3AF] mb-1">Website URL</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[#9CA3AF] mb-1">Industry</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-[#9CA3AF] mb-1">Estimated Size</label>
                  <input
                    type="text"
